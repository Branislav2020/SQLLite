...
SELECT 
    ...
    LFA1.LIFNR,
    LFA1.ZSYSNAME,
    BKPF.BLART as BLART, --not necessary to add labels if it is not repeated name in select
    BKPF.BUDAT as BUDAT, --not necessary to add labels if it is not repeated name in select
    BKPF.CPUDT as CPUDT  --not necessary to add labels if it is not repeated name in select
FROM "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_RSEG" AS RSEG
--I would start with INNER JOIN and after that put left joins
INNER JOIN "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_BKPF" AS BKPF ON 1=1
    ...
    AND RSEG.MANDT  IN ('200') --I would suggest CAST(RSEG.MANDT AS INT) = 200
INNER JOIN "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_BSEG" AS BSEG ON 1=1
    ...
    AND BSEG.DMBTR*-1 >= 0
	--I WOULD  USE  straight inner join with the condition at the end
INNER JOIN "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_LFA1" AS LFA1 ON 1=1
/*INNER JOIN (SELECT * FROM "DTAG_DEV_CSBI_CELONIS_DATA"."dtag.dev.csbi.celonis.data.elog::V_LFA1" AS TEMP -- i wouldn't use *, but put there only columns I need
...
LEFT JOIN "DTAG_DEV_CSBI_CELONIS_WORK"."dtag.dev.csbi.celonis.app.p2p_elog::__P2P_REF_CASES" AS EKPO ON 1=1
    ...
    AND RSEG.EBELN || RSEG.EBELP = EKPO.EBELN || EKPO.EBELP --would replace it with AND (RSEG.EBELN = EKPO.EBELN || RSEG.EBELP =EKPO.EBELP)
WHERE LFA1.LIFNR > '020000000' --not sure how LFA1.LIFNR looks like, but I would be careful when comparing to string
;